...
class NFFGToolBox(object):
  ...
  def _is_port_finishing_flow (TAG, port):
    """
    Determines whether this port finishes the flow sequence of TAG.

    :param TAG: TAG to look for
    :type TAG: str
    :param port: Port object
    :type port: :any:`Port`
    :return: finishes the flow sequence or not
    :rtype: bool
    """
  ...
  @staticmethod
  def get_TAGs_of_starting_flows (port, sg_hops_to_be_ignored):
    """
    ...
    """
    for fr in port.flowrules:
      if fr.hop_id not in sg_hops_to_be_ignored:
        for action in fr.action.split(";"):
          command_param = action.split("=")
          if command_param[0] == "TAG":
  ...
  def retrieve_mapped_path (TAG, nffg, starting_port):
    ...
    :type starting_port: :any:`Port`
    :return: list of found link and bandwidth
    :rtype: tuple
    """
    edge_list = []
    tag_list = TAG.split("|")
